...
Scene 1 (0.0–3.4s): five tiles cluster→outward expansion (`center-outward-expansion`) into a staggered grid, each landing on its spoken cue (CLI → catalog → Studio → cloud rendering get mono labels under their tiles); riser runs underneath.
Scene 2 (3.4–6.08s): tiles dim back as the star count value-scaled counter (`counting-dynamic-scale`) climbs 0→44K hero-center with "stargazers · tldraw · TanStack" beneath; count lands with impact-soft; holds on the number — the energetic peak resolves still.
