...
description: >
  ...
  as scarce "voltage", hairline ink elevation (no heavy shadow), paperMono for all
  display + paperMono body + IBM Plex Mono for the index/code voice on a warm-navy code surface,
...
typography:
  # — reading + chrome ramp —
  body:    { fontFamily: "paperMono", cqw: 1.5, weight: 400, lineHeight: 1.5 }
  lead:    { fontFamily: "paperMono", cqw: 2.08, weight: 400, lineHeight: 1.5 }
  card-title:{ fontFamily: "paperMono", cqw: 2.3, weight: 500, lineHeight: 1.25, tracking: "-0.005em" }
  button:  { fontFamily: "paperMono", cqw: 1.46, weight: 500, lineHeight: 1.0 }
  tag-upper:{ fontFamily: "paperMono", cqw: 1.35, weight: 500, tracking: "0.18em", upper: true }
  ...
  # — display ramp (paperMono 400, sentence case, negative tracking. Renderer embeds only 400/700 — author at 400; italic is the synthesized slant) —
  headline:{ fontFamily: "paperMono", cqw: 4.6, weight: 400, lineHeight: 1.06, tracking: "-0.018em" }
  quote-pull:{ fontFamily: "paperMono", cqw: 5.0, weight: 400, lineHeight: 1.12, tracking: "-0.012em", italic: true }
  display-italic:{ fontFamily: "paperMono", cqw: 6.7, weight: 400, lineHeight: 1.05, tracking: "-0.012em", italic: true }
  display:{ fontFamily: "paperMono", cqw: 7.3, weight: 400, lineHeight: 1.02, tracking: "-0.022em" }
  number-hero:{ fontFamily: "paperMono", cqw: 9.4, weight: 400, lineHeight: 0.95, tracking: "-0.025em" }
  display-cover:{ fontFamily: "paperMono", cqw: 9.9, weight: 400, lineHeight: 0.98, tracking: "-0.028em" }
...
components:
  ...
  number-lockup:
    ...
    description: "Hero stat — a paperMono figure paired with a IBM Plex Mono unit (200K, +1,204, −318, 17 files). Figure is serif; the unit is ALWAYS mono, never the serif."
  pull-quote:
    typography: "{typography.quote-pull} (paperMono italic) + {typography.tag-upper} cite"
    description: "A commit message, a reviewer line, or the thesis. paperMono italic, small paperMono uppercase cite beneath."
  section-rule:
...
- **Fonts** — already set to **paperMono** (display) / **Inter** (body); ignore any preset font name lingering in prose.
...
Three editorial voices, each in its own face: **paperMono** carries every display moment — covers,
headlines, pull-quotes, big stat numerals — at large display sizes with gentle negative tracking;
its **italic** is the expressive register. **paperMono** carries body, leads, card titles, buttons, and
...
- **paperMono** (sentence case, negative-tracked) for all display; **paperMono** body/chrome; **IBM Plex Mono** index + code.
...
- **Squint** — one paperMono display moment dominates at 3–6× its neighbor.
...
- **Type** — paperMono sentence-case display (negative-tracked); paperMono 400 body; IBM Plex Mono kickers (uppercase 0.16em, coral ✱) + code.
...
Two ramps. The **reading/chrome ramp** (paperMono `body` 1.5cqw / `lead` 2.08cqw weight 400; JetBrains
Mono `kicker`/`mono-label` in px) carries copy + chrome; the **display ramp** (paperMono `headline` 4.6cqw
...
- **paperMono display is sentence case** (NOT title case, NOT uppercase), weight 400, negative-tracked (−0.012..−0.028em); reach for **italic** when the line is a stance or a definition. **paperMono body** sentence case weight 400. **IBM Plex Mono** kickers UPPERCASE 0.16em with the coral ✱. No uppercase serif, no sans headline, no serif label.
...
- **number-lockup** — paperMono figure + mono unit (the PR `+N / −M`, `200K`, file counts). **pull-quote** — paperMono italic + cite (a commit message / reviewer line).
...
### 1 · Cover (identity · move: oversized paperMono · cream)

**Ground** `{colors.cream}`, `slide-pad`. **Composes** kicker-spike, display-cover, lead, mono-label index. **Focal** a 2–3 line paperMono `display-cover` (sentence case, ink) under a coral ✱ kicker. **Chrome** mono index strip (repo · branch). **Accent** the single coral ✱. **Fixed** paperMono 400 sentence case, hairline, cream ground. **Free** title, the mono index, layout + how full the frame runs. **Density** free.

### 2 · Statement (statement · move: single paperMono line · cream or navy)

**Ground** `{colors.cream}` (or `{colors.navy}` for gravity). **Composes** kicker-spike, display, optional lead. **Focal** one 2-line paperMono `display` carrying the change in a sentence — reach for **italic** if it's a stance. **Chrome** mono kicker. **Accent** none — the serif carries it (or one coral word). **Fixed** sentence-case serif. **Free** the line, ground, layout + density. **Density** free.
...
- Set all display in **paperMono, sentence case**, negative-tracked; **paperMono 400** body; **IBM Plex Mono** kickers (uppercase, 0.16em, coral ✱) + code.
...
- Pair a paperMono figure with a **mono unit** for every stat; render code via the **`code-*` blocks** on the navy surface.
...
- No uppercase or title-case paperMono display; no sans headline; no serif label; no serif-set numeric unit.
...
- **Squint** — one paperMono display moment dominates.
...
- **Type** — paperMono sentence-case display (negative-tracked); paperMono 400 body; IBM Plex Mono kickers (uppercase 0.16em, coral ✱) + code; ≥1.4cqw floor.
...
```html
<style>
@font-face{font-family:"paperMono";font-weight:400;font-style:normal;font-display:block;src:url("assets/fonts/paperMono-Regular.woff2") format("woff2");}
</style>
```
