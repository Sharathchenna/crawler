...
public enum HoardKeychain {
  ...
  private static let cookieAccount = "access-cookies"
  ...
  // MARK: - Cloudflare Access session cookies (captured from the web login)

  public struct AccessCookie: Codable, Sendable {
    public var name: String
    public var value: String
    public init(name: String, value: String) { self.name = name; self.value = value }
  }

  /// Persist the `CF_*` cookies the browser login produced so `URLSession`
  /// can present the same session the website uses to clear the Access edge.
  public static func saveAccessCookies(_ cookies: [AccessCookie]) throws {
    try save(account: cookieAccount, data: try JSONEncoder().encode(cookies))
  }

  public static func loadAccessCookies() -> [AccessCookie] {
    guard let data = load(account: cookieAccount) else { return [] }
    return (try? JSONDecoder().decode([AccessCookie].self, from: data)) ?? []
  }

  public static func deleteAccessCookies() {
