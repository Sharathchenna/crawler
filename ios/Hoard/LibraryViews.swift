...
final class ItemListModel {
  ...
  var filter = "all"
  var collection: ItemCollection = .all
  var query = ""
...
struct ItemListContent: View {
  ...
  var body: some View {
    VStack(spacing: 0) {
      if showCollections {
        Picker("Collection", selection: $model.collection) {
          ForEach(ItemCollection.allCases) { c in Text(c.rawValue).tag(c) }
        }
        .pickerStyle(.segmented)
        .padding(.horizontal)
        .padding(.top, 8)
        .onChange(of: model.collection) { Task { await load() } }
      }
      if showFilterChips {
    ...
    NavigationStack {
      ...
      .toolbar {
        ToolbarItem(placement: .topBarTrailing) {
          Button { showCapture = true } label: { Image(systemName: "plus.circle.fill") }
            .tint(HoardTheme.accentHi)
            .accessibilityLabel("Quick capture")
        }
        ToolbarItem(placement: .topBarLeading) {
          Button { showSettings = true } label: { Image(systemName: "gearshape") }
            .tint(HoardTheme.muted)
            .accessibilityLabel("Settings")
        }
      }
      .sheet(isPresented: $showCapture) {
        QuickCaptureSheet(onSaved: {})
...
struct InboxView: View {
  ...
  var body: some View {
    NavigationStack {
      ...
      .sheet(isPresented: $showCapture) {
        QuickCaptureSheet(onSaved: { refreshCount() })
          .presentationDetents([.medium, .large])
      }
    }
    .task { refreshCount() }
