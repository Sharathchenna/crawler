...
struct AuthView: View {
  ...
  var body: some View {
      ...
      ScrollView {
        VStack(spacing: 0) {
          ...
          Text("hoard.")
            ...
            .tracking(-0.76)
          Text("Sign in the same way as the website.")
            .font(.inter(15))
          ...
          VStack(spacing: 12) {
            // Primary: the same browser SSO the website uses. One tap → Access
            // login → the app mints and stores a token for you.
            Button {
              errorMessage = nil
              showWebLogin = true
            } label: {
              HStack(spacing: 8) {
                ...
                Text("Sign in with Cloudflare Access").font(.inter(15, weight: .medium))
              }
            ...
            Button {
              ...
              Text(showManual ? "Hide token sign-in" : "Sign in with a token instead")
                .font(.mono(12)).foregroundStyle(HoardTheme.muted)
            }
            .buttonStyle(.plain)
    ...
    .sheet(isPresented: $showWebLogin) {
      NavigationStack {
        AccessLoginView(baseURL: HoardConfig.baseURL) { token, webEmail, cookies in
          session.signInWithAccess(token: token, email: webEmail, cookies: cookies)
          showWebLogin = false
        }
        ...
        .toolbar {
          ToolbarItem(placement: .cancellationAction) {
            Button("Cancel") { showWebLogin = false }
          }
        }
      }
    }
