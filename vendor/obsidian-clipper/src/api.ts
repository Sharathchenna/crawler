...
export async function clip(options: ClipOptions): Promise<ClipResult> {
	...
	// Hoard adaptation: pass the full Document, not documentElement. Verified
	// against defuddle@0.19.3 + linkedom: element input extracts to empty
	// (no title/content) while document input works; document internals
	// (e.g. documentElement.getAttribute for lang detection) need the real
	// tree. Everything downstream is unchanged.
